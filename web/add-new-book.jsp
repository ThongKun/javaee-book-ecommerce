...
<html>
    ...
    <body>
        ...
        <form class="user-form" action="add-new-book" method="POST" enctype="multipart/form-data">
            ...
            Description <textarea type="text" name="description">${requestScope.description}</textarea><br/>
            ...
            Category<select name="categoryId">
                <c:forEach items="${requestScope.CATEGORIES}" var="item">
                    <option value="${item.id}" <c:if test="${item.id eq requestScope.categoryId}">selected</c:if> >${item.name}</option>
                </c:forEach>
            </select><br/>
            ...
            Quantity <input type="number" name="quantity" value="${requestScope.quantity > 1 ? requestScope.quantity : 1}" /><br/>
